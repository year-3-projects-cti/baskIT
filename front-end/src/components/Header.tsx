    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-card/85 backdrop-blur-xl shadow-soft supports-[backdrop-filter]:bg-card/70">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 py-3">
          ...
          {/* Navigation */}
          <nav className="hidden md:flex flex-1 items-center justify-center gap-2 rounded-full border border-white/20 bg-white/70 px-1 py-1 text-sm shadow-soft backdrop-blur">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
        ...
        <div className="flex flex-col gap-3 pb-4 md:flex-row md:items-center md:justify-between">
          ...
          <Button asChild className="w-full md:w-auto rounded-full bg-gradient-hero px-6 shadow-medium hover:opacity-90">
